...
<div id="more_events">
    <?php if (isset($more_events) && sizeof($more_events) > 0) { ?>
        ...
        <?php foreach ($more_events as $event) { ?>
            ...
            <a href="<?= yii\helpers\Url::to(['healthcare-events/' . str_replace('/', '-', $event['categories'][0]) . '/' . str_replace('/', '-', implode('-', $event['sub_categories'])), 'eid' => (string) $event['_id'], 'store' => $nearest_store_number]) ?>">
